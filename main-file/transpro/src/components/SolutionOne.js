import React from "react";

const SolutionOne = () => {
  return (
    <>
      {/* Start Solution area  */}
      <div className='solution_area text-center'>
        <div className='container'>
          <div className='row justify-content-center'>
            <div className='col-lg-6'>
              <div className='section-title   text-center'>
                <span className='subtitles'>Experience</span>
                <h2 className='title'>Our Solution</h2>
                <p>
                  Lose away off why half led have near bed. At engage simple
                  father of period others except. My giving do summer of though
                  narrow marked at. Spring formal no county ye waited.
                </p>
              </div>
            </div>
          </div>
          <div className='row  justify-content-center'>
            <div className='col-lg-2 col-md-4 col-sm-4'>
              <div className='solution-item'>
                <div className='solution-image'>
                  <img
                    src='https://transpro.wowtheme7.com/assets/img/solution/01.png'
                    className='img-fluid'
                    alt='solution image'
                  />
                </div>
                <h4 className='title'>Export Logistics</h4>
              </div>
            </div>
            <div className='col-lg-2 col-md-4  col-sm-4'>
              <div className='solution-item'>
                <div className='solution-image'>
                  <img
                    src='https://transpro.wowtheme7.com/assets/img/solution/02.png'
                    className='img-fluid'
                    alt='solution image'
                  />
                </div>
                <h4 className='title'>Import Logistics</h4>
              </div>
            </div>
            <div className='col-lg-2 col-md-4   col-sm-4'>
              <div className='solution-item'>
                <div className='solution-image'>
                  <img
                    src='https://transpro.wowtheme7.com/assets/img/solution/03.png'
                    className='img-fluid'
                    alt='solution image'
                  />
                </div>
                <h4 className='title'>Cargo Express</h4>
              </div>
            </div>
            <div className='col-lg-2 col-md-4   col-sm-4'>
              <div className='solution-item'>
                <div className='solution-image'>
                  <img
                    src='https://transpro.wowtheme7.com/assets/img/solution/04.png'
                    className='img-fluid'
                    alt='solution image'
                  />
                </div>
                <h4 className='title'>Information Express</h4>
              </div>
            </div>
            <div className='col-lg-2 col-md-4   col-sm-4'>
              <div className='solution-item'>
                <div className='solution-image'>
                  <img
                    src='https://transpro.wowtheme7.com/assets/img/solution/05.png'
                    className='img-fluid'
                    alt='solution image'
                  />
                </div>
                <h4 className='title'>Warehousing And Distribution</h4>
              </div>
            </div>
            <div className='col-lg-2 col-md-4   col-sm-4'>
              <div className='solution-item'>
                <div className='solution-image'>
                  <img
                    src='https://transpro.wowtheme7.com/assets/img/solution/06.png'
                    className='img-fluid'
                    alt='solution image'
                  />
                </div>
                <h4 className='title'>Brokerage Services</h4>
              </div>
            </div>
          </div>
        </div>
      </div>
      {/* End Solution area  */}
    </>
  );
};

export default SolutionOne;
